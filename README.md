...
Trunks is a Neovim git client. It has some similarities to [vim-fugitive](https://github.com/tpope/vim-fugitive). This plugin has two commands:
...
- The `:Trunks` command, which provides subcommands for git features that aren't already git commands. For example, `:Trunks vdiff` opens a vim diff in a vertical split, `:Trunks clog` sends `git log` entries to the quickfix list, etc.
...
- Most `:G` commands will display their output in terminal mode. This is a powerful concept:
  ...
  - Some terminal extensions for git, like [delta](https://github.com/dandavison/delta) or [diff-so-fancy](https://github.com/so-fancy/diff-so-fancy), will work just like they do in the terminal.
...
# Configuration
<details>
<summary>These are default values</summary>
...
```lua
{
    ...
    config = function()
        ...
        vim.g.trunks_configuration = {
            ...
            diff_popup = {
                keymaps = { -- Run git diff with various options
                    vdiff = "d",
                    hdiff = "h",
                    term = "t",
                },
            ...
            log = {
                ...
                keymaps = {
                    ...
                    reword = "rw", -- Reword commit under cursor
...
# Tests
All tests run with [busted](https://lunarmodules.github.io/busted/).
...
Thanks to Tim Pope and the maintainers of [vim-fugitive](https://github.com/tpope/vim-fugitive), an absolutely incredible vim plugin that heavily influenced Trunks.
