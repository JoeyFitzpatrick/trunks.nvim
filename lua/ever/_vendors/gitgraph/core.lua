...
function M._gitgraph(raw_commits, opt, sym, fields)
    ...
    ---@class GenerateConnectroRowParams
    ---@field prev_commit_row I.Row
    ---@field prev_connector_row I.Row
    ---@field commit_row I.Row
    ---@field commit_loc integer
    ---@field curr_commit I.Commit
    ---@field next_commit I.Commit?
        ...
        if #curr_commit.parents > 0 then
            ...
            local function reserve_remainder(rem_parents)
                ...
                for _, h in ipairs(rem_parents) do
                    ...
                    if not j then
                        j = next_vacant_j(connector_cells, commit_loc)
            ...
            local is_bi_crossing, bi_crossing_safely_resolveable =
                utils.get_is_bi_crossing(commit_row, connector_row, next_commit)
    ...
    ---@param input_commits table<string, I.Commit>
    ---@param input_sorted_commits string[]
    ---@return I.Row[]
    local function straight_j(input_commits, input_sorted_commits)
        local graph = {} ---@type I.Row[]

        for i, c_hash in ipairs(input_sorted_commits) do
            -- get the input parameters
            local curr_commit = input_commits[c_hash]
            local next_commit = input_commits[input_sorted_commits[i + 1]]
            ...
            if i < #input_sorted_commits then
    ...
    ---@param input_graph I.Row[]
    ---@param options I.DrawOptions
    ---@return string[]
    ---@return I.Highlight[]
    ---@return integer?
    local function graph_to_lines(options, input_graph)
        ---@type integer?
        local head_loc = 1
        ...
        local width = 0
        for _, row in ipairs(input_graph) do
        ...
        for idx = 1, #input_graph do
            local proper_row = input_graph[idx]
            ...
            if options.mode ~= "test" then
                ...
                if c then
                    ...
                    local branch_names = #c.branch_names > 0 and ("(%s)"):format(table.concat(c.branch_names, " | "))
                        or nil
                    ...
                    if options.mode == "debug" then
                        local parents = ""
                        for _, h in ipairs(input_graph[idx].commit.parents) do
                            local p = commits[h]
                            parents = parents .. (p and p.msg or "?")
                        end

                        local children = ""
                        for _, h in ipairs(input_graph[idx].commit.children) do
                            local p = commits[h]
    ...
    for i = 2, #graph - 1 do
        ...
        for j = 1, #row.cells, 2 do
            ...
            if not this.is_commit and not this.connector then
                ...
                -- local trivial_continuation = (not row_commit_is_child)
                -- and (new_columns < 1 or ach == tch or acc == GVER)
    ...
    for i = 2, #graph, 2 do
        ...
        for j = 1, #row.cells, 2 do
            ...
            if this.connector ~= GVER then
                ...
                local symb_n = 0
                for index, b in ipairs({ l, r, u, d }) do
                    if b then
                        nn = nn + 1
                        symb_n = symb_n + bit.lshift(1, 4 - index)
                    end
                end
                ...
                -- placement of commit horizontally, only relevant if this is a connector row
                -- and if the cell is not immediately above or below the commit
                ---@type 'l' | 'r' | nil
                local clh_above = nil
