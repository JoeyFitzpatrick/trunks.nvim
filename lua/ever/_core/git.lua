...
M.is_staged = function(status)
    return vim.tbl_contains(require("lua.ever._constants.git_status").STAGED_STATUSES, status)
end
...
M.is_modified = function(status)
    return vim.tbl_contains(require("lua.ever._constants.git_status").MODIFIED_STATUSES, status)
end
...
M.is_deleted = function(status)
    return vim.tbl_contains(require("lua.ever._constants.git_status").DELETED_STATUSES, status)
end
...
M.is_renamed = function(status)
    return vim.tbl_contains(require("lua.ever._constants.git_status").RENAMED_STATUSES, status)
end
