...
---
---@module 'ever._cli.cli_subcommand'
---
...
---@class ever.NamespaceExecuteArguments
---    The expected data that's passed to any `set_execute` call in ever.
...
---@class ever.CompleteData
---    The data that gets passed when `ever.Subcommand.complete` is called.
---@field input argparse.Results
---    All information that was found from parsing some user's input.

---@class ever.RunData
---    The data that gets passed when `ever.Subcommand.run` is called.
---@field input argparse.Results
---    All information that was found from parsing some user's input.

---@class ever.Subcommand
...
---@field complete (fun(data: ever.CompleteData): string[])?
...
---@field run (fun(data: ever.SubcommandRun): nil)?
---    The function to run when the subcommand is called.

---@class ever.SubcommandRun
...
---@alias ever.ParserCreator fun(): cmdparse.ParameterParser

---@alias ever.Subcommands table<string, plugin_template.Subcommand | fun(): cmdparse.ParameterParser>

--- Check if `full` contains `prefix` + whitespace.
---
---@param full string Some full text like `"Ever blah"`.
---@param prefix string The expected starting text. e.g. `"Ever"`.
...
---
---@param text string Some full text like `"Ever blah"`.
---@param prefix string The expected starting text. e.g. `"Ever"`.
---@param subcommands ever.Subcommands All allowed commands.
---
local function _get_subcommand_completion(text, prefix, subcommands)
    local argparse = require("ever._cli.argparse")
    ...
    if not subcommands[subcommand_key] then
        vim.notify(
            string.format(
                'Ever: Unknown command "%s". Please check your spelling and try again.',
                subcommand_key
            ),
    ...
    ---@cast subcommand ever.Subcommand
    ...
    if subcommand.complete then
        ...
        if result == nil or vim.islist(result) then
            ...
            vim.notify(
                string.format(
                    'ever: Subcommand / Arguments "%s / %s" must be a string[]. Got "%s".',
                    subcommand,
                    arguments,
                    vim.inspect(result)
                )
...
local function _run_subcommand(parser, text)
    ...
    ---@type fun(data: ever.NamespaceExecuteArguments): nil
    ...
    vim.notify(
        string.format(
            'Ever: Command "%s" parsed "%s" text into "%s" namespace but no `execute` '
...
---
---@param subcommands ever.Subcommands
---    All registered commands for `ever` to possibly modify.
---
...
---
---@param parser_creator ever.ParserCreator
...
---
---@param prefix string The command to exclude from auto-complete. e.g. `"Ever"`.
---@param subcommands ever.Subcommands All allowed commands.
...
function M.make_subcommand_triager(subcommands)
    ...
    local function _runner(opts)
        ...
        if not subcommand then
            vim.notify("Ever: Unknown command: " .. subcommand_key, vim.log.levels.ERROR)
    ...
    local function runner(opts)
        ...
        local help_message = require("ever._cli.cmdparse.help_message")
