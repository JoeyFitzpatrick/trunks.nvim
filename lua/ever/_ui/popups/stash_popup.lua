...
local function set_keymaps(bufnr)
    local keymaps = require("ever._ui.keymaps.base").get_keymaps(bufnr, "stash_popup", { popup = true })
        ...
        set("n", keys, function()
            ...
            vim.ui.input({ prompt = "Stash message: " }, function(input)
                ...
                if input:match("^%s*$") then
                    ...
                    vim.cmd(command .. " -m " .. require("ever._core.texter").surround_with_quotes(input))
                end
            end)
        end, keymap_opts)
    end
